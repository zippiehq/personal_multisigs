...
contract ZippieSmartWalletERC20 is ZippieAccount {
        ...
        returns (bool)
    {
        ...
        require(
            IZippieMerchantRegistry(_zippieMerchantRegistry).merchantOwner(merchantId) != address(0), 
            "Merchant owner not set"
        );

        require(
            IZippieMerchantRegistry(_zippieMerchantRegistry).merchantOwner(merchantId) == msg.sender, 
