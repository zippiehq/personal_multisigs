...
contract ZippieMultisigWallet{
    ...
    function checkAndTransferFrom(address[] multisigAndERC20Contract, address[] allSignersPossible, uint8 m, uint8[] v, bytes32[] r, bytes32[] s, uint256 nonce, address recipient, uint256 amount) public {
        ...
        checkAndTransferFrom_Card(addresses, allSignersPossible, signatureRequirements, v, r, s, nonce, amount, cardDigests);
    }

    // Almost same descriptions as at top but structured (combined) to prevent stack too deep errors
    // addresses -- multisig address, erc20 contract address, recipient
    ...
    function checkAndTransferFrom_Card(address[] addresses, address[] allSignersPossible, uint8[] m, uint8[] v, bytes32[] r, bytes32[] s, uint256 nonce, uint256 amount, bytes32[] cardDigests) public {
